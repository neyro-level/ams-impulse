...
export default async function ResearchPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const filterForm = <FilterBar surface="plain" className="md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_180px_160px_180px_auto_auto]">
    ...
    <NativeSelect name="sort" defaultValue={sort} aria-label="Сортировка"><NativeSelectOption value="updated">Сначала обновлённые</NativeSelectOption><NativeSelectOption value="title">По названию</NativeSelectOption><NativeSelectOption value="status">По статусу</NativeSelectOption><NativeSelectOption value="cost">По расчётной стоимости</NativeSelectOption></NativeSelect>
    <Button type="submit">Применить</Button>
    <Link href={`/tools/research/?organizationId=${encodeURIComponent(selected.organizationId)}&projectId=${encodeURIComponent(selected.id)}`} className="inline-flex min-h-11 items-center justify-center rounded border border-input px-4 text-sm font-semibold text-foreground">Сбросить</Link>
  </FilterBar>;
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="min-w-0" aria-label="Список исследований">
        {result.items.length ? <>
          <div className="hidden rounded-panel border border-border bg-card md:block"><Table><TableCaption className="sr-only">Исследования выбранного проекта</TableCaption><TableHeader><TableRow><TableHead>Название</TableHead><TableHead>Статус</TableHead><TableHead>Запросы</TableHead><TableHead>Обновлено</TableHead><TableHead>Последний запуск</TableHead><TableHead className="text-right">Действие</TableHead></TableRow></TableHeader><TableBody>{result.items.map((record) => <TableRow key={record.id}><TableCell className="font-semibold text-foreground">{record.title}</TableCell><TableCell><StatusBadge label={label[record.status] ?? record.status} tone={tone[record.status] ?? "neutral"} /></TableCell><TableCell>{record.queryCount}</TableCell><TableCell>{new Date(record.updatedAt).toLocaleDateString("ru-RU")}</TableCell><TableCell>{record.lastRun ? <><p className="font-medium text-foreground">{rubles(record.lastRun.allocatedCostKopecks ?? record.lastRun.estimatedCostKopecks)}</p><p className="text-xs text-muted-foreground">{new Date(record.lastRun.createdAt).toLocaleDateString("ru-RU")}</p></> : "—"}</TableCell><TableCell className="text-right"><Link href={detailHref(record.id)} className="inline-flex min-h-10 items-center font-semibold text-primary">Открыть</Link></TableCell></TableRow>)}</TableBody></Table></div>
          <div className="grid gap-3 md:hidden">{result.items.map((record) => <article key={record.id} className="rounded-card border border-border bg-card p-4"><div className="flex items-start justify-between gap-3"><h2 className="font-semibold text-foreground">{record.title}</h2><StatusBadge label={label[record.status] ?? record.status} tone={tone[record.status] ?? "neutral"} /></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-muted-foreground">Запросы</dt><dd className="mt-1 font-medium">{record.queryCount}</dd></div><div><dt className="text-muted-foreground">Обновлено</dt><dd className="mt-1">{new Date(record.updatedAt).toLocaleDateString("ru-RU")}</dd></div><div className="col-span-2"><dt className="text-muted-foreground">Последний запуск</dt><dd className="mt-1">{record.lastRun ? `${new Date(record.lastRun.createdAt).toLocaleDateString("ru-RU")} · ${rubles(record.lastRun.allocatedCostKopecks ?? record.lastRun.estimatedCostKopecks)}` : "Запусков пока нет"}</dd></div></dl><Link href={detailHref(record.id)} className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded bg-primary px-4 text-sm font-semibold text-primary-foreground">Открыть исследование</Link></article>)}</div>
