...
export default function NotFound() {
  return (
    <main className={`theme-public ${styles.landing} relative isolate flex min-h-dvh items-center overflow-hidden bg-[var(--ch-bg-deepest)] px-5 py-14 text-[var(--ch-white)] sm:px-6`}>
      ...
      <div className="absolute left-1/2 top-1/2 -z-10 -translate-x-1/2 -translate-y-1/2 select-none text-public-ghost font-extrabold text-[var(--ch-ghost-ondark)]" aria-hidden>
        404
      </div>
      ...
      <div className="mx-auto w-full max-w-not-found text-center">
        ...
        <h1 className="mt-5 text-public-page-title font-extrabold text-[var(--ch-white)]">
        ...
        <nav className="mt-12 grid gap-3 text-left sm:grid-cols-3" aria-label="Доступные разделы">
          {quickLinks.map((item, index) => (
            <Link key={item.href} href={item.href} className="group min-h-36 border border-[var(--ch-border-subtle)] bg-[var(--ch-surface-faint)] p-5 transition hover:-translate-y-0.5 hover:border-[var(--ch-accent)]/65 hover:bg-[var(--ch-surface-faint-hover)]">
              <span className="text-public-micro font-bold text-[var(--ch-accent)]">{String(index + 1).padStart(2, "0")}</span>
