...
function SortLink({ field, label, query }: { field: ProjectTableQuery["sort"]; label: string; query: ProjectTableQuery }) {
  ...
  return <Link className="inline-flex min-h-11 items-center gap-1 text-left text-xs font-semibold uppercase tracking-wide text-secondary-text" href={projectHref(query, { page: 1, sort: field, direction })}>{label}{active ? query.direction === "asc" ? <ChevronUp className="size-3.5" aria-hidden /> : <ChevronDown className="size-3.5" aria-hidden /> : <ChevronDown className="size-3.5 opacity-30" aria-hidden />}</Link>;
}
...
export function ProjectTable({ result, options, query }: { result: ProjectListResult; options: ProjectFormOptions; query: ProjectTableQuery }) {
  const columns = useMemo<ColumnDef<typeof impulseTableFeatures, ProjectListItem, unknown>[]>(() => [
    { accessorKey: "name", header: () => <SortLink field="name" label="Проект" query={query} />, cell: ({ row }) => <p className="font-semibold text-foreground">{row.original.name}</p> },
    { accessorKey: "organizationName", header: "Организация", cell: ({ row }) => <span className="text-sm text-secondary-text">{row.original.organizationName}</span> },
    ...
    { accessorKey: "updatedAt", header: () => <SortLink field="updatedAt" label="Обновлён" query={query} />, cell: ({ row }) => <time className="block whitespace-nowrap text-right text-sm tabular-nums text-secondary-text" dateTime={row.original.updatedAt}>{new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium" }).format(new Date(row.original.updatedAt))}</time> },
