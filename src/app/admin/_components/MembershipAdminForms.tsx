...
function MembershipEditCard({ item }: { item: MembershipListItem }) {
  ...
  const submitUpdate = updateForm.handleSubmit(async (values) => { const result = await updateMembershipAction(values); if (!result.ok) { applyFieldErrors(result.error.fieldErrors, updateForm.setError); setUpdateFeedback(feedbackFrom(result)); return; } setUpdateFeedback({ kind: "success", message: "Доступ обновлён" }); router.refresh(); });
  const submitRemove = removeForm.handleSubmit(async (values) => { const result = await removeMembershipAction(values); if (!result.ok) { applyFieldErrors(result.error.fieldErrors, removeForm.setError); setRemoveFeedback(feedbackFrom(result)); return; } setRemoveFeedback({ kind: "success", message: "Доступ отозван" }); router.refresh(); });
  return <SectionCard title={item.userName} description={`${item.userEmail} · ${item.organizationName}`}><div className="space-y-4"><form className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]" onSubmit={submitUpdate}><input type="hidden" {...updateForm.register("organizationId")} /><input type="hidden" {...updateForm.register("membershipId")} /><input type="hidden" {...updateForm.register("version", { valueAsNumber: true })} /><FormField error={updateForm.formState.errors.tenantRole?.message} label="Уровень доступа" required><SelectInput options={tenantRoleOptions} {...updateForm.register("tenantRole")} /></FormField><div className="self-end"><SubmitRow busy={updateForm.formState.isSubmitting} feedback={updateFeedback} label="Сохранить" onRefresh={() => router.refresh()} pendingLabel="Сохраняем…" variant="outline" /></div></form><form className="flex flex-wrap items-center gap-3 border-t border-border pt-4" onSubmit={submitRemove}><input type="hidden" {...removeForm.register("organizationId")} /><input type="hidden" {...removeForm.register("membershipId")} /><input type="hidden" {...removeForm.register("version", { valueAsNumber: true })} /><SubmitRow busy={removeForm.formState.isSubmitting} feedback={removeFeedback} label={`Отозвать доступ: ${item.organizationName}`} onRefresh={() => router.refresh()} pendingLabel="Отзываем…" variant="ghost" /></form></div></SectionCard>;
}
