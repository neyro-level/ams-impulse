...
export function PlatformAdminTable({ resource, query, rows, total, pageSize, sortOptions }: { resource: string; query: PlatformAdminPageQuery; rows: PlatformAdminDisplayRow[]; total: number; pageSize: number; sortOptions: Array<{ field: PlatformAdminSortField; label: string }> }) {
  ...
  const columns = useMemo<ColumnDef<typeof impulseTableFeatures, PlatformAdminDisplayRow, unknown>[]>(() => [
    { accessorKey: "primary", header: () => <SortLink field={primarySort?.field ?? "name"} label={primarySort?.label ?? "Запись"} query={query} resource={resource} />, cell: ({ row }) => <div><p className="font-semibold text-foreground">{row.original.primary}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{row.original.secondary}</p></div> },
    ...
    { accessorKey: "updatedAt", header: () => <SortLink field="updatedAt" label="Обновлено" query={query} resource={resource} />, cell: ({ row }) => row.original.updatedAt ? <time className="block whitespace-nowrap text-right text-sm tabular-nums text-secondary-text" dateTime={row.original.updatedAt}>{new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(row.original.updatedAt))}</time> : <span className="block text-right text-sm text-muted-foreground">Нет proof</span> },
  ], [primarySort, query, resource]);
  const pageCount = Math.max(1, Math.ceil(total / pageSize));
  return <AdminDataTable ariaLabel="Список записей" columns={columns} data={rows} filtersActive={Boolean(query.search)} emptyTitle="Записей пока нет" emptyDescription="Создайте первую запись в форме ниже." filteredEmptyDescription="Измените поисковый запрос или сбросьте фильтр." page={query.page} pageCount={pageCount} previousHref={buildPlatformAdminPageHref(resource, query, { page: Math.max(1, query.page - 1) })} nextHref={buildPlatformAdminPageHref(resource, query, { page: Math.min(pageCount, query.page + 1) })} mobileRenderer={(row) => <article className="rounded-card border border-border bg-card p-4"><div className="flex items-start justify-between gap-3"><h2 className="font-semibold text-foreground">{row.primary}</h2><StatusBadge label={row.status} tone={statusTone(row.status)} /></div><p className="mt-2 text-xs leading-5 text-muted-foreground">{row.secondary}</p>{row.updatedAt ? <time className="mt-4 block text-xs tabular-nums text-muted-foreground" dateTime={row.updatedAt}>{new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(row.updatedAt))}</time> : <p className="mt-4 text-xs text-muted-foreground">Нет proof</p>}</article>} />;
}
