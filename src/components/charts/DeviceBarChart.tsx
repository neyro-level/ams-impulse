...
export function DeviceBarChart({ data, timezone }: DeviceBarChartProps) {
  ...
  return (
    <AnalyticsCard title="Устройства" description="Как распределяются визиты и конверсия между типами устройств." units="визиты и проценты" timezone={timezone} summary={<p className="text-sm text-secondary-text">Всего визитов: <strong className="tabular-nums text-foreground">{formatInteger(visitsTotal)}</strong></p>}>
      ...
      <ul className="mt-4 grid gap-2 text-sm text-secondary-text sm:grid-cols-2">
        {data.map((item) => (
          <li key={item.device} className="rounded border border-border bg-muted px-3 py-2">
            <span className="font-semibold text-foreground">{item.device}</span>: {formatInteger(item.visits)} визитов, {formatPercent(item.conversionRate)} конверсия
          </li>
        ))}
      </ul>
    </AnalyticsCard>
