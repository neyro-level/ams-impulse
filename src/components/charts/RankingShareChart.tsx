...
export function RankingShareChart({ ranking, timezone }: RankingShareChartProps) {
  ...
  return (
    <AnalyticsCard title="Доля запросов в Топ-3 и Топ-10" description={`Как меняется доля видимых в Яндексе запросов. Всего отслеживается: ${ranking.queryCount}.`} period={`${ranking.history[0]?.date} — ${ranking.history.at(-1)?.date}`} units="проценты и запросы" timezone={timezone} summary={<div className="flex flex-wrap gap-5 text-sm text-secondary-text"><span><span className="mr-2 inline-block size-2.5 rounded-full bg-chart-2" />Топ-10: <strong className="tabular-nums text-foreground">{ranking.top10Share.toFixed(1)}%</strong></span><span><span className="mr-2 inline-block size-2.5 rounded-full bg-chart-4" />Топ-3: <strong className="tabular-nums text-foreground">{ranking.top3Share.toFixed(1)}%</strong></span></div>}>
