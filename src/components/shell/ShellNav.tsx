...
function navigationStateClass(active: boolean, muted?: boolean) {
  if (active) {
    return "border border-sidebar-active-border bg-sidebar-active font-medium text-sidebar-foreground";
  }
  return muted
    ? "border border-transparent text-sidebar-muted hover:bg-sidebar-surface"
    : "border border-transparent text-sidebar-foreground hover:bg-sidebar-hover";
}
...
function NavigationLink({ href, label, active, muted, collapsed, onNavigate, child = false }: { href: string; label: string; active: boolean; muted?: boolean; collapsed: boolean; onNavigate?: () => void; child?: boolean }) {
  const link = (
    <Link
      ...
      className={`flex items-center rounded text-sm transition-colors ${collapsed ? "min-h-11 justify-center px-2" : child ? "min-h-10 gap-2.5 px-2.5" : "min-h-11 gap-3 px-3"} ${navigationStateClass(active, muted)}`}
    >
    ...
    <nav className={collapsed ? "space-y-4" : "space-y-6"} aria-label="Навигация по отчётам">
      {sections.map((section) => (
        <div key={section.title} className="space-y-2">
          {section.title && !collapsed ? <p className="px-2 text-eyebrow font-semibold uppercase text-sidebar-muted">{section.title}</p> : null}
            ...
            {section.items.map((item) => {
              ...
              return (
                <li key={item.href} className="space-y-1">
                  <div className={`flex items-center rounded transition-colors ${navigationStateClass(active)} ${active ? "border-l-2 border-l-[var(--sidebar-project-accent)]" : ""}`}>
                    ...
                    <button
                      ...
                      className="mr-1 grid size-9 shrink-0 place-items-center rounded text-sidebar-muted transition-colors hover:bg-sidebar-surface-hover hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring"
                  ...
                  {expanded ? (
                    <ul id={regionId} className="ml-4 space-y-0.5 border-l border-sidebar-group-border py-1 pl-2">
