...
export function LegalDocument({ eyebrow, title, description, version, effectiveDate, children }: LegalDocumentProps) {
  return (
    <main className={`theme-public ${landingStyles.landing} min-h-screen bg-[var(--ch-bg-page)] text-[var(--ch-text-primary)]`}>
      <header className="bg-[var(--ch-bg-deepest)] text-[var(--ch-white)]">
        <div className="mx-auto w-full max-w-legal px-container pb-16 pt-6 sm:px-container-wide sm:pb-20 sm:pt-8">
          ...
          <div className="mt-16 max-w-4xl sm:mt-24">
            <p className="text-public-label font-bold uppercase text-[var(--ch-accent)]">{eyebrow}</p>
            <h1 className="mt-5 text-public-page-title font-extrabold text-[var(--ch-white)]">{title}</h1>
      ...
      <article className={`${styles.prose} mx-auto w-full max-w-legal px-container py-14 text-left sm:px-container-wide sm:py-20`}>
