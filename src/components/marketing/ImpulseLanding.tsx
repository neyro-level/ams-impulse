...
export function ImpulseLanding({ loginRequested, oauthLoginRequested = false }: { loginRequested: boolean; oauthLoginRequested?: boolean }) {
  return <main className="theme-public impulse-landing min-h-screen overflow-hidden bg-[var(--ch-bg-deepest)] text-[var(--ch-white)]"><section className="relative isolate min-h-screen"><div className={`${styles.grid} absolute inset-0 -z-20`} aria-hidden /><div className={`${styles.atmosphere} absolute inset-0 -z-10`} aria-hidden /><PublicHeader loginRequested={loginRequested} oauthLoginRequested={oauthLoginRequested} /><div className="mx-auto grid w-full max-w-[1360px] gap-12 px-5 pb-12 pt-14 sm:px-6 sm:pt-20 lg:min-h-[calc(100vh-96px)] lg:grid-cols-[minmax(0,7fr)_minmax(360px,5fr)] lg:items-center lg:gap-16 lg:pb-20 lg:pt-10"><HeroCopy /><HeroVisual /></div></section><ServiceMechanism /><ReportProof /><SiteFooter /></main>;
}
