CREATE INDEX "Run_organizationId_projectId_status_estimateExpiresAt_idx"
  ON "research"."Run"("organizationId", "projectId", "status", "estimateExpiresAt");
  ...
  WITH chargeable AS (
    SELECT
      CASE
        ...
        WHEN run."status" IN ('SUCCEEDED', 'FAILED')
    ...
    WHERE run."organizationId" = target_organization_id
      ...
      AND run."status" IN ('AWAITING_CONFIRMATION', 'QUEUED', 'RUNNING', 'SUCCEEDED', 'FAILED')
